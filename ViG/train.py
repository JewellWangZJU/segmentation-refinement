...
def main(args):
    ...
    for exp_id in range(args.folds):
        ...
        if args.dataset == 'isic':
            ...
            overall_id = list(range(len(glob.glob(args.data_root + '/images/*.png'))))  # Assuming images are named 0.png to 185.png
            test_id = overall_id[3*exp_id:3*(exp_id+1)] if exp_id < args.folds-1 else overall_id[3*exp_id:]
            train_id = list(set(overall_id)-set(test_id))
            trainset = MyDataset_CRACKS(args, train_root=args.data_root, pat_ls=train_id, mode='train')
            validset = MyDataset_CRACKS(args, train_root=args.data_root, pat_ls=test_id, mode='test')
